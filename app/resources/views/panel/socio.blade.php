    ...
    <div class="row g-3 g-md-4 panel-acciones">
        ...
        <div class="col-12 col-md-6">
            <a href="{{ route('incidencias.index') }}" class="accion-card accion-card--alt" data-conectividad>
                <span class="accion-card__icon" aria-hidden="true">
                    &#9776;
                    @if ($incidenciasEnProceso > 0)
                        <span class="accion-card__badge">{{ $incidenciasEnProceso }}</span>
                    @endif
                </span>
                <span class="accion-card__title">Mis incidencias</span>
                @if ($incidenciasEnProceso > 0)
                    <span class="visually-hidden">{{ $incidenciasEnProceso }} {{ $incidenciasEnProceso === 1 ? 'incidencia en proceso' : 'incidencias en proceso' }}</span>
                @endif
